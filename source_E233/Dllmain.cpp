...
ATS_API ATS_HANDLES WINAPI Elapse(ATS_VEHICLESTATE vehicleState, int *panel, int *sound)
{
	...
	if (DispType == 9 && snp2Output == 0) //9号千代田線系統
	{
		...
		if (ETIMS != 0)
		{
			...
			//panel[300] = g_tims.NextBlinkLamp; //次駅停車表示灯
			...
			/*
			panel[343] = g_tims.HiddenLine[0] ? 0 : g_9n.McKey != 6 ? g_tims.DispSESta[0] : g_tims.Station[0]; //駅名表示1
			panel[344] = g_tims.HiddenLine[1] ? 0 : g_9n.McKey != 6 ? g_tims.DispSESta[1] : g_tims.Station[1]; //駅名表示2
			panel[345] = g_tims.HiddenLine[2] ? 0 : g_9n.McKey != 6 ? g_tims.DispSESta[2] : g_tims.Station[2]; //駅名表示3
			panel[346] = g_tims.HiddenLine[3] ? 0 : g_9n.McKey != 6 ? g_tims.DispSESta[3] : g_tims.Station[3]; //駅名表示4
			panel[347] = g_tims.HiddenLine[4] ? 0 : g_9n.McKey != 6 ? g_tims.DispSESta[4] : g_tims.Station[4]; //駅名表示5
			*/
			...
			panel[4] = g_tims.HiddenLine[0] ? 0 : g_9n.McKey == 6 ? g_tims.Arrive[0][1] : DispType != 5 ? 0 : g_9n.SetTrainPass(g_tims.DispSESta[0]); //到着時刻1M
			...
			panel[10] = g_tims.HiddenLine[1] ? 0 : g_9n.McKey == 6 ? g_tims.Arrive[1][1] : DispType != 5 ? 0 : g_9n.SetTrainPass(g_tims.DispSESta[1]); //到着時刻2M
			...
			panel[11] = g_tims.HiddenLine[2] ? 0 : g_9n.McKey == 6 ? g_tims.Arrive[2][1] : DispType != 5 ? 0 : g_9n.SetTrainPass(g_tims.DispSESta[2]); //到着時刻3M
			...
			panel[12] = g_tims.HiddenLine[3] ? 0 : g_9n.McKey == 6 ? g_tims.Arrive[3][1] : DispType != 5 ? 0 : g_9n.SetTrainPass(g_tims.DispSESta[3]); //到着時刻4M
			...
			panel[16] = g_tims.HiddenLine[4] ? 0 : g_9n.McKey == 6 ? g_tims.Arrive[4][1] : DispType != 5 ? 0 : g_9n.SetTrainPass(g_tims.DispSESta[4]); //到着時刻5M
			...
			panel[195] = g_9n.McKey != 6 ? 0 : g_tims.HiddenLine[0] ? 0 : g_tims.Leave[0][2]; //発車時刻1S
			//panel[153] = g_tims.HiddenLine[1] ? 0 : g_tims.Leave[1][0]; //発車時刻2H
			panel[196] = g_9n.McKey != 6 ? 0 : g_tims.HiddenLine[1] ? 0 : g_tims.Leave[1][1]; //発車時刻2M
			panel[197] = g_9n.McKey != 6 ? 0 : g_tims.HiddenLine[1] ? 0 : g_tims.Leave[1][2]; //発車時刻2S
			...
			else
			{
				...
				panel[173] = g_9n.McKey != 6 ? 0 : g_tims.BeforeTime[0]; //直前採時駅着時刻H
				...
				panel[49] = g_9n.McKey == 6 ? 0 : g_9n.DepartSta; //乗車駅
...
*/
	...
	else //TIMS_new_9互換機能
	{
		if (EMeter != 0) //メーター表示器を出力
		{
			...
			panel[35] = g_meter.BrakeDelay; //ブレーキ指令
			panel[40] = g_meter.BrakeDelay == g_emgBrake ? 1 : 0; //非常ブレーキ
		}
		if (EVmeter != 0) //電圧を出力
		{
			//電圧類
